...
// created also by me
function PerceptionQuickDDDate($name, $date=NULL, $format=NULL, $markup=NULL, $start_year=1900, $end_year=2100)
{
   // normalize params
...
// safe redirect
function perception_redirect($url) {
	echo "<meta http-equiv='refresh' content='0;url=$url' />"; 
	exit;
}

// new line for CSV
function perception_define_newline() {
	$unewline = "\r\n";
...
function perception_get_mime_type() {
...
function perception_display_alerts() {
	global $error, $success;
	
	if(!empty($_SESSION['perception_flash']))
	{
		echo "<div class='perception-alert'><p>".$_SESSION['perception_flash']."</p></div>";
		unset($_SESSION['perception_flash']);
	}
	
	if(!empty($error)){
		echo '<div class="perception-error"><p>'.$error.'</p></div>';
	}
	
	if(!empty($success)){
		echo '<div class="perception-success"><p>'.$success.'</p></div>';
	}
}

function perception_datetotime($date) {
	list($year, $month, $day) = explode("-",$date);
	return mktime(1, 0, 0, $month, $day, $year);
}

// function to conditionally add DB fields
function perception_add_db_fields($fields, $table) {
		global $wpdb;
...
function perception_enqueue_datepicker() {
	$locale_url = get_option('perception_locale_url');	
...
function perception_int_array($value) {
...
function perception_strip_tags($content) {
