...
class PSPerceptionLMSMultiUser {
	static function manage() {
		global $wpdb, $wp_roles;
		$roles = $wp_roles->roles;
		
		// this sets the setting of a selected role
		if(!empty($_POST['config_role']) and check_admin_referer('perception_role_settings')) {
			...
			update_option('perception_role_settings', serialize($role_settings));	
			do_action('perception-role-settings-saved', $_POST['role_key']);
		} // end config_role
		...
		if(@file_exists(get_stylesheet_directory().'/perception/multiuser.html.php')) require get_stylesheet_directory().'/perception/multiuser.html.php';
		else require PERCEPTION_PATH."/views/multiuser.html.php";
	}
	...
	static function check_access($what, $noexit = false) {
		...
		else wp_die(__('You are not allowed to do this.', 'perception'));
	}
