<?php
class PSPerceptionLMSGradebookController {
	...
	// @param $atts - the shortcode attributes when the function is called by the [perception-gradebook] shortcode
	static function manage($in_shortcode = false, $public = false, $atts = null) {
		...
		if(!$public and !is_user_logged_in()) {
			_e('You need to be logged in to view this content.', 'perception');
			return false;
		}
		...
		if(!empty($_GET['course_id'])) {
			if(!$public and !current_user_can('perception_manage')) {
				// in this case students see only their own gradebook		
				return self :: my_gradebook($_GET['course_id']);
			}			
			$students_sql = '';
			if(!$in_shortcode or !$public) do_action('perception-check-permissions', 'course', $_GET['course_id']);
			...
			$students = $wpdb->get_results($wpdb->prepare("SELECT tU.*, tS.status as perception_status, tS.grade as course_grade 
		 	...
		 	if(!$in_shortcode or !$public) $students = apply_filters('perception-gradebook-students', $students, $_GET['course_id']);
		 				
			// select student-lessons and student-homeworks for this course so we can have the grades			
			$student_lessons = $wpdb->get_results("SELECT * FROM ".PERCEPTION_STUDENT_LESSONS."
				WHERE lesson_id IN (".implode(',',$lids).") AND student_id IN (".implode(',', $stids).")
				ORDER BY id");
				
			$student_homeworks = $wpdb->get_results("SELECT tSH.*, tH.lesson_id as lesson_id FROM ".PERCEPTION_STUDENT_HOMEWORKS." tSH
				JOIN ".PERCEPTION_HOMEWORKS." tH ON tH.id = tSH.homework_id
		...
		if(@file_exists(get_stylesheet_directory().'/perception/gradebook.html.php')) require get_stylesheet_directory().'/perception/gradebook.html.php';
		else require(PERCEPTION_PATH."/views/gradebook.html.php");
	} // end manage();
	...
	static function my_gradebook($course_id = 0, $in_shortcode = false) {
		...
		JOIN ".PERCEPTION_COURSES." tSC ON tSC.course_id = tC.ID AND (tSC.status = 'enrolled' OR tSC.status = 'completed')
		AND tSC.user_id=%d
		...
		if(!empty($_GET['course_id']) and in_array($_GET['course_id'], $cids)) {
			...
			AND tM.meta_value = %d
			LEFT JOIN ".PERCEPTION_STUDENT_LESSONS." tSL ON tSL.student_id=%d AND tSL.lesson_id=tP.ID
			...
			$homeworks = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".PERCEPTION_HOMEWORKS." WHERE course_id=%d ORDER BY id", $this_course->ID));
			$hids = array(0);
		...
		if(@file_exists(get_stylesheet_directory().'/perception/my-gradebook.htm.php')) require get_stylesheet_directory().'/perception/my-gradebook.html.php';
		else require(PERCEPTION_PATH."/views/my-gradebook.html.php");
	} // end my_gradebook
	...
	static function auto_grade_course($course_id, $student_id) {
		...
		if(get_option('perception_use_grading_system') == '') return false;

		$auto_grade = get_post_meta($course_id, 'perception_auto_grade', true);
		...
		foreach($lessons as $lesson) {
			$lesson_grade = $wpdb->get_var($wpdb->prepare("SELECT grade FROM ".PERCEPTION_STUDENT_LESSONS." 
		...
		update_option('perception_testing', "TOTAL $total NUM: $num_graded_lessons");
		
		$grades = get_option('perception_grading_system');		
		...
		$wpdb->query($wpdb->prepare("UPDATE ".PERCEPTION_COURSES." SET grade=%s WHERE course_id=%d AND user_id=%d", $target_grade, $course_id, $student_id));
		do_action('perception_graded_course', $student_id, $course_id, $target_grade);
