<?php
class PSPerceptionLMSSearchController {
	// when Perception search is done we have to search only in courses and/or lessons (if user is enrolled)
   static function pre_get_posts($query) {
   	global $wpdb, $user_ID;   	
   	if(empty($_GET['perception_search'])) return $query;
  
   	// search in Perception. If user is not logged in, limit the $post_type to perception_course, otherwise include also perception_lesson
   	if(is_user_logged_in()) $post_types = array('perception_course', 'perception_lesson');
   	else $post_types = array('perception_course');
   ...
   static function filter_where($where) {
		...
		if(empty($_GET['perception_search'])) return $where;

		$post_ids = array(-1);		
		
		// course limit?
		if(!empty($_GET['perception_course_id'])) {
			$post_ids[] = intval($_GET['perception_course_id']);			
			
			// if I am enrolled, add the lessons
			$is_enrolled = $wpdb -> get_var( $wpdb->prepare("SELECT id FROM ".PERCEPTION_COURSES."
			WHERE user_id = %d AND course_id = %d", $user_ID, $_GET['perception_course_id']));
			
			if($is_enrolled) {
				if(empty($_GET['perception_lesson_id'])) {
					$_lesson = new PSPerceptionLMSLessonModel();
					$lessons = $_lesson->select($_GET['perception_course_id']);
					foreach($lessons as $lesson) $post_ids[] = $lesson->ID;
				}
				else {
					// add the lesson if it really belongs to the course
					$course_id = get_post_meta($_GET['perception_lesson_id'], 'perception_course', true);					
					if($course_id == $_GET['perception_course_id']) $post_ids[] = $_GET['perception_lesson_id'];
				}
			}							
		}
		else {
			...
			$course_ids = $wpdb->get_results("SELECT ID FROM {$wpdb->posts} 
				WHERE post_type='perception_course' AND post_status = 'publish' ");
			...
			JOIN ".PERCEPTION_COURSES." tSC ON tP.ID = tSC.course_id 
   ...
   static function form() {
		...
		if(is_user_logged_in()) {	
	   	...
	   	if(!empty($_GET['perception_course_id'])) {
	   		foreach($lessons as $lesson) {
	   			if($lesson->course_id == $_GET['perception_course_id']) $current_lessons[] = $lesson;
				}
			}
   	...
   	if(@file_exists(get_stylesheet_directory().'/perception/search-form.html.php')) require get_stylesheet_directory().'/perception/search-form.html.php';
		else require(PERCEPTION_PATH."/views/search-form.html.php");
	}
}