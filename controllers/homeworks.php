<?php
class PSPerceptionLMSHomeworkController {
	...
	static function submit_solution($in_shortcode = false) {
		...
		list($homework, $course, $lesson) = PSPerceptionLMSHomeworkModel::full_select($_GET['id']);
		
		if(!self :: submit_solution_prerequizites($course, $lesson, $homework)) return false;
		
		$file_upload_progress = get_option('perception_file_upload_progress');
		
		// now submit
		if(!empty($_POST['ok'])) {
			if(empty($_POST['content']) and empty($_FILES['files']['tmp_name'][0]) and empty($_POST['solution_files_uploaded'])) wp_die(__('You cannot submit a solution without any text or files.', 'perception'));			
			
			// avoid duplicates
			$exists = $wpdb->get_var($wpdb->prepare("SELECT id FROM ".PERCEPTION_STUDENT_HOMEWORKS."
				WHERE student_id=%d AND homework_id=%d AND content=%s", $user_ID, $homework->id,
				$_POST['content']));
			if(!$exists) {
				$wpdb->query($wpdb->prepare("INSERT INTO ".PERCEPTION_STUDENT_HOMEWORKS." SET
				...
				// print_r($_SESSION['perception_file_errors']);
				// these lines are in case of using Ajax uploads. Then we have stored the errors in session
				if(empty($total_size_error) and !empty($_SESSION['perception_file_errors'][0])) $total_size_error = $_SESSION['perception_file_errors'][0];
				if(empty($file_errors) and !empty($_SESSION['perception_file_errors'][1])) $file_errors = $_SESSION['perception_file_errors'][1];
				if(empty($file_size_errors) and !empty($_SESSION['perception_file_errors'][2])) $file_size_errors = $_SESSION['perception_file_errors'][2];
				if(empty($file_not_uploaded_errors) and !empty($_SESSION['perception_file_errors'][3])) $file_not_uploaded_errors = $_SESSION['perception_file_errors'][3];
				
				// any uploaded files without a solution for this user & homework? We need to update them in case they are uploaded by Ajax
				$wpdb->query($wpdb->prepare("UPDATE ".PERCEPTION_SOLUTION_FILES." SET solution_id=%d
					WHERE solution_id=0 AND student_id=%d AND homework_id=%d", $solution_id, $user_ID, $homework->id)); 				
			}	 			
			
			do_action('perception_submitted_solution', $user_ID, $homework->id);
			...
			$wpdb->query($wpdb->prepare("INSERT INTO ".PERCEPTION_HISTORY." SET
				...
				$user_ID, sprintf(__('Submitted solution to assignment "%s"', 'perception'), $homework->title), $homework->id, $course->ID));
				
			// unset session errors 
			if(isset($_SESSION['perception_file_errors'])) unset($_SESSION['perception_file_errors']);
			
			if(@file_exists(get_stylesheet_directory().'/perception/solution-submitted.php')) require get_stylesheet_directory().'/perception/solution-submitted.php';
			else require(PERCEPTION_PATH."/views/solution-submitted.php");
		}
		else {			 
			 if(@file_exists(get_stylesheet_directory().'/perception/submit-solution.php')) require get_stylesheet_directory().'/perception/submit-solution.php';
			 else require(PERCEPTION_PATH."/views/submit-solution.php");
		}		
	}
	
	// change uploads dir
	static function change_upload_dir($dirs) {
		$protected_folder = get_option('perception_protected_folder');
	...
	static function upload_files($homework = null, $solution_id = 0) {
		...
		$allowed_file_types = get_option('perception_allowed_file_types');
		...
		$total_size_limit = get_option('perception_homework_size_total');
		if(intval($total_size_limit) > 0) {
			...
			if($total_size > $total_size_limit) {
				$_SESSION['perception_file_errors'] = array($total_size, null, null, null); // return total size error
				return $_SESSION['perception_file_errors'];
			}
		} // end checking size
		
		
		$file_size_limit = get_option('perception_homework_size_per_file');
		...
		if($homework->accept_files and !empty($_FILES['files']['tmp_name'][0])) {		
			...
			foreach($_FILES['files']['tmp_name'] as $cnt => $tmp_name) {
				...
				if(get_option('perception_store_files_filesystem') != 1) $file_blob = file_get_contents($tmp_name);
				else {													
				...
				$wpdb->query($wpdb->prepare("INSERT INTO ".PERCEPTION_SOLUTION_FILES." SET
		...
		$_SESSION['perception_file_errors'] = array($total_size_error, $file_errors, $file_size_errors, $file_not_uploaded_errors);		
	...
	static function submit_solution_prerequizites($course, $lesson, $homework) {
		...
		// am I enrolled?
		if(!PSPerceptionLMSStudentModel::is_enrolled($user_ID, $course->ID)) wp_die(__('You are not enrolled in this course!',
			'perception'));
		...
		if(!$exists) {
			echo '<p>'.__('You need to read this lesson before you can submit solutions to assignments.', 'perception').'</p>';
			return false; 
		}		
			
		// unsatisfied lesson completion requirements?
		$not_completed_ids = PSPerceptionLMSLessonModel :: unsatisfied_complete_requirements($lesson);
		if(!empty($not_completed_ids)) {
			 $content = '<p>'.__('Before submitting solutions on this lesson you must complete the following lessons:','perception').'</p>';			 
			 $content	.= '<ul>';
		...
		if($homework->limit_by_date and 
			...
			printf('<p>'.__('Solutions will be accepted between %s and %s.', 'perception'),
	...
	static function view($in_shortcode = false) {
		...
		if(!current_user_can('perception_manage') and $student_id!=$user_ID) wp_die(__('You are not allowed to see these solutions', 'perception'));
		...
		$multiuser_access = PSPerceptionLMSMultiUser :: check_access('homework_access', $noexit);
		if($multiuser_access == 'own' and $homework->editor_id != $user_ID and $student_id != $user_ID) wp_die(__('You are not allowed to see these solutions', 'perception'));
		...
		$grades = explode(",", stripslashes(get_option('perception_grading_system')));
		// give grade on a solution
		if($use_grading_system and !empty($_POST['grade_solution']) and current_user_can('perception_manage')) {
		   PSPerceptionLMSHomeworkModel :: set_grade($_POST['grade'], $_POST['id']);
		}
		...
		if(@file_exists(get_stylesheet_directory().'/perception/view-solutions.php')) require get_stylesheet_directory().'/perception/view-solutions.php';
		else require(PERCEPTION_PATH."/views/view-solutions.php");
	}
	...
	static function solution_files($homework, $solution) {
		...
		$files = $wpdb->get_results($wpdb->prepare("SELECT id, file FROM ".PERCEPTION_SOLUTION_FILES." WHERE solution_id=%d ORDER BY id", $solution->id));
	...
	static function view_all() {
		...
		if($multiuser_access == 'own' and $homework->editor_id != $user_ID) wp_die(__('You are not allowed to see these solutions', 'perception'));
		...
		$solutions = $wpdb -> get_results($wpdb->prepare("SELECT tH.*, tU.user_login as user_login 
			FROM ".PERCEPTION_STUDENT_HOMEWORKS." tH JOIN {$wpdb->users} tU ON tH.student_id = tU.ID
	...
	static function change_solution_status($lesson, $student_id = NULL) {
		...
		if(!current_user_can('perception_manage')) wp_die(__('You are not allowed to do this', 'perception'));
		
		$solution = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".PERCEPTION_STUDENT_HOMEWORKS." WHERE id=%d", $_POST['solution_id']));
		if(!$student_id)  $student_id = $solution->student_id;
		$homework = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".PERCEPTION_HOMEWORKS." WHERE id=%d", $solution->homework_id));
		
		$multiuser_access = 'all';
		$multiuser_access = PSPerceptionLMSMultiUser :: check_access('homework_access');
		if($multiuser_access == 'own' and $homework->editor_id != $user_ID) wp_die(__('You are not allowed to see these solutions', 'perception'));
			
		$wpdb->query($wpdb->prepare("UPDATE ".PERCEPTION_STUDENT_HOMEWORKS." SET
			status=%s WHERE id=%d", $_POST['status'], $_POST['solution_id']));
			
		do_action('perception_change_solution_status', $student_id, $_POST['solution_id'], $_POST['status']);	
		
		// insert in history
		$course_id = get_post_meta($homework->lesson_id, 'perception_course', true);	
		$wpdb->query($wpdb->prepare("INSERT INTO ".PERCEPTION_HISTORY." SET
			user_id=%d, date=CURDATE(), datetime=NOW(), action='solution_processed', value=%s, num_value=%d, course_id=%d",
			$student_id, sprintf(__('Solution to assignment %s was %s', 'perception'), $homework->title, $_POST['status']), $_POST['solution_id'], $course_id));
		
		// award points?
		if($_POST['status']=='approved' and get_option('perception_use_points_system')) {			
			if($homework->award_points) {
				PerceptionPoint :: award($student_id, $homework->award_points, sprintf(__('Received %d points for completing assignment "%s".', 'perception'), 
					$homework->award_points, $homework->title), 'homework', $homework->id);
			}
		}
		
		// maybe complete the lesson if the status is approved 				
		if($_POST['status']=='approved' and PSPerceptionLMSLessonModel::is_ready($lesson->ID, $student_id)) {
			PSPerceptionLMSLessonModel::complete($lesson->ID, $student_id);
		}		
	...
	static function download_solution() {
		...
		$solution = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".PERCEPTION_STUDENT_HOMEWORKS." WHERE id=%d", intval($_GET['id'])));
		$file = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".PERCEPTION_SOLUTION_FILES." 
			WHERE solution_id=%d AND id=%d", $solution->id, intval($_GET['file_id'])));
		
		if(empty($solution->fileblob) and empty($file->fileblob) and empty($file->filepath)) wp_die(__("There is nothing to download.", 'perception'));
		
		if(!current_user_can('perception_manage') and $user_ID != $solution->student_id) wp_die(__('You can download only your own solutions.', 'perception'));
