...
class PerceptionToDo {
	...
	static function manager_todo() {
		...
		$enrollments = $wpdb->get_results("SELECT COUNT(tSC.id) as cnt, tC.post_title as course,
			tC.ID as course_id 
			FROM ".PERCEPTION_COURSES." tSC JOIN {$wpdb->posts} tC ON tC.ID = tSC.course_id
		...
		$approvals = $wpdb->get_results("SELECT tU.user_login as user_login, tU.ID as student_id,
			...
			FROM ".PERCEPTION_STUDENT_LESSONS." tSL
			JOIN {$wpdb->posts} tL ON tL.ID = tSL.lesson_id
			JOIN {$wpdb->users} tU ON tU.ID = tSL.student_id	
			JOIN {$wpdb->postmeta} tM ON tM.meta_key = 'perception_course'	
		...
		$homeworks = $wpdb->get_results("SELECT tH.title as title, tU.user_login as user_login,
			...
			FROM ".PERCEPTION_STUDENT_HOMEWORKS." tSH
		...
		if(@file_exists(get_stylesheet_directory().'/perception/managers-todo.html.php')) include get_stylesheet_directory().'/perception/managers-todo.html.php';
		else include(PERCEPTION_PATH."/views/managers-todo.html.php");
	} // end managers_todo()
}