...
class PerceptionTrack {
	static function visit($what, $id, $user_id) {
		...
		$exists = $wpdb->get_var( $wpdb->prepare("SELECT id FROM ".PERCEPTION_VISITS." 
			WHERE user_id=%d AND $field=%d AND date = CURDATE()", $user_id, $id));
			
		if($exists) {
			$wpdb->query( $wpdb->prepare("UPDATE ".PERCEPTION_VISITS." SET
				visits = visits+1 WHERE id=%d", $exists));
		}	
		else {
			$wpdb->query( $wpdb->prepare("INSERT INTO ".PERCEPTION_VISITS." SET
	...
	static function get_visits($what, $id) {
		...
		$visits = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".PERCEPTION_VISITS." 
