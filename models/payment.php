<?php
class PerceptionPayment {
	static $pdt_mode = false;	
	static $pdt_response = '';	
	
	// handle Paypal IPN request
	static function parse_request($wp) {
		// only process requests with "perception=paypal"
	   if (array_key_exists('perception', $wp->query_vars) 
	            && $wp->query_vars['perception'] == 'paypal') {
	        self::paypal_ipn($wp);
	   }	
	}
	...
	static function paypal_ipn($wp = null) {
		global $wpdb;
		echo "<!-- PERCEPTIONCOMMENT paypal IPN -->";
		
		// print_r($_GET);
		// read the post from PayPal system and add 'cmd'
		$pdt_mode = get_option('perception_use_pdt');
		if(!empty($_GET['tx']) and !empty($_GET['perception_pdt']) and get_option('perception_use_pdt')==1) {
			// PDT			
			$req = 'cmd=_notify-synch';
			$tx_token = strtoupper($_GET['tx']);
			$auth_token = get_option('perception_pdt_token');
		...
		$paypal_sandbox = get_option('perception_paypal_sandbox');
	...
	static function paypal_ipn_verify($pp_response) {
	   ...
	   $paypal_email = get_option("perception_paypal_id");
      ...
      // process payment
	   ...
	   // check txn_id
	   $txn_exists = $wpdb->get_var($wpdb->prepare("SELECT paycode FROM ".PERCEPTION_PAYMENT_METHOD."
		...
		if(empty($txn_id)) $txn_id_okay = true; 
			...
			if( self :: $pdt_mode) perception_redirect(get_permalink($_GET['course_id']));
		...
		if($_POST['mc_currency']==get_option("perception_currency")) {
		...
		$fee = get_post_meta($_GET['course_id'], 'perception_fee', true);
      $course = get_post($_GET['course_id']);		
		...
		if($payment_completed and $txn_id_okay and $receiver_okay and $payment_currency_okay 
			...
			do_action('perception-paid', $_GET['user_id'], $fee, "course", $_GET['course_id'], $is_school);	
				
			// enroll accordingly to course settings - this will be placed in a method once we 
			// have more payment options
			$enroll_mode = get_post_meta($_GET['course_id'], 'perception_enroll_mode', true);	
			if(!PSPerceptionLMSStudentModel :: is_enrolled($_GET['user_id'], $_GET['course_id']))  {
			...
			else perception_redirect(get_permalink($_GET['course_id']));
		}  
	...
	static function log_and_exit($msg) {
		...
		$errorlog=get_option("perception_errorlog");
		$errorlog = $msg."\n".$errorlog;
		update_option("perception_errorlog",$errorlog);
		
		// if we are in Paypal PDT mode just echo and don't exit
		if(self :: $pdt_mode) {
			echo $msg;
			if(get_option('perception_debug_mode')) echo "<br>Full response: ".self :: $pdt_response;
			return true;
		}
