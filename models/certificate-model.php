<?php
class PSPerceptionLMSCertificateModel {
	function add($vars) {
		global $wpdb, $user_ID;
		
		$this->prepare_vars($vars);
		
		$wpdb->query($wpdb->prepare("INSERT INTO ".PERCEPTION_CERTIFICATES." SET
	...
	function edit($vars, $id) {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".PERCEPTION_CERTIFICATES." SET
	...
	function prepare_vars(&$vars) {		
		...
		$vars['course_ids'] = perception_int_array($vars['course_ids']);
		$vars['title'] = sanitize_text_field($vars['title']);
		$vars['content'] = perception_strip_tags($vars['content']);
		...
		$vars['expired_message'] = perception_strip_tags($vars['expired_message']);
	...
	function delete($id) {
		...
		$wpdb->query($wpdb->prepare("DELETE FROM ".PERCEPTION_CERTIFICATES." WHERE id=%d", $id));		
	}
	...
	function student_certificates($student_id) {
		...
		$certificates = $wpdb->get_results($wpdb->prepare("SELECT tC.*, tM.id as my_id, tM.date as date 
			FROM ".PERCEPTION_CERTIFICATES." tC JOIN ".PERCEPTION_STUDENT_CERTIFICATES." tM ON tM.certificate_id = tC.id
	...
	function complete_course($course_id, $student_id) {
		...
		$certificates = $wpdb->get_results("SELECT * FROM ".PERCEPTION_CERTIFICATES." 
		...
		foreach($certificates as $certificate) {
			...
			if($courses_completed) {
				$wpdb->query($wpdb->prepare("INSERT IGNORE INTO ".PERCEPTION_STUDENT_CERTIFICATES." SET
					student_id=%d, certificate_id=%d, date=%s", $student_id, $certificate->id, date("Y-m-d")));
					
				do_action('perception_achieved_certificate', $student_id, $certificate->id);	
			}
