<?php
class PSPerceptionLMSStudentModel {
	...
	static function manage() {
		 ...
		 if(!empty($_GET['course_id'])) {
                ...
                self :: cleanup($_GET['course_id'], $_GET['student_id']); 
					...
					perception_redirect("admin.php?page=perception_students&course_id=".intval($_GET['course_id'])."&status=".sanitize_text_field($_GET['status']));		
				}		 	
				
				// mass cleanup
				if(!empty($_POST['mass_cleanup']) and check_admin_referer('perception_manage_students')) {
					if($multiuser_access  == 'view') wp_die(__('You are not allowed to do this.', 'perception'));
					$ids = empty($_POST['student_ids']) ? array() : perception_int_array($_POST['student_ids']);
				...
				if((!empty($_POST['mass_approve']) or !empty($_POST['mass_reject'])) and check_admin_referer('perception_manage_students')) {
					if($multiuser_access  == 'view') wp_die(__('You are not allowed to do this.', 'perception'));
					$ids = empty($_POST['student_ids']) ? array() : perception_int_array($_POST['student_ids']);
				...
				if(!empty($_GET['enroll'])) {
					 ...
					 // user exists?
					 if(empty($student->ID)) $error = __('Sorry, I cannot find user with this email or user handle.', 'perception');
					 
					 // allowed to use PSPerception?
					 if(!$error and !user_can($student->ID, 'administrator') and !user_can($student->ID, 'perception')) {
					 	$error = __("This user's role does not allow them to use PSPerception LMS. You'll have either to change their role or allow the role work with the LMS from the Settings page", 'perception');
					 }	
					 
					 // already enrolled?
					 if(!$error) {
						 $is_enrolled = $wpdb->get_var($wpdb->prepare("SELECT id FROM ".PERCEPTION_COURSES."
						 	WHERE user_id = %d AND course_id = %d", $student->ID, intval($_GET['course_id'])));
						 if($is_enrolled) $error = __('This user is already enrolled in the course', 'perception');
					 }
					 ...
					 if(empty($error)) {
							...
							$course_id = intval($_GET['course_id']);					 	
					 		...
					 		$wpdb -> query($wpdb -> prepare("INSERT INTO ".PERCEPTION_COURSES." SET
					 		...
					 		$success = __('User successfully enrolled in the course', 'perception');	
					 		...
					 		$wpdb->query($wpdb->prepare("INSERT INTO ".PERCEPTION_HISTORY." SET
								...
								$student->ID, sprintf(__('Enrolled in course %s. Status: %s', 'perception'), stripslashes($course->post_title), 'enrolled'), 
								$_GET['course_id'], $_GET['course_id']));
					 		
					 		// do_action('perception_enrolled_course', $student->ID, $_GET['course_id'], true);
					 	  do_action('perception_admin_enrolled_course', $student->ID, $_GET['course_id'], true);	
				...
				if(!empty($_GET['change_status'])) {
					...
					perception_redirect("admin.php?page=perception_students&course_id=".intval($_GET['course_id']));					 							 	
				}				
            ...
            if(!empty($_GET['filter_tags'])) {
				...
				$filter_sql = apply_filters('perception-students-filter', '');		 		
		 		
		 		$students = $wpdb->get_results("SELECT SQL_CALC_FOUND_ROWS tU.*, 
		 			tS.status as perception_status, tS.grade as grade, tU.display_name as display_name, tS.id as scid,
		 			tS.enrollment_date as enrollment_date, tS.completion_date as completion_date, tS.tags as tags 
			 		FROM {$wpdb->users} tU JOIN ".PERCEPTION_COURSES." tS 
				...
				foreach($students as $student) {
					if($student->perception_status == 'pending') $any_pending = true;
				}
		 			 		
		 		// select student - to - lesson relations
		 		$completed_lessons = $wpdb->get_results("SELECT * FROM ".PERCEPTION_STUDENT_LESSONS."
				...
				$use_grading_system = get_option('perception_use_grading_system');		 	
		 		...
		 		if(!empty($_GET['export'])) {
		 			$newline = perception_define_newline();
				
					$titlerow = __("Student name", 'perception').','.__("Student email", 'perception').',';
					...
					$titlerow .= __('Status in course', 'perception');
					
					if($use_grading_system) {
						$titlerow .= ','.__('Final grade', 'perception');
					}
					...
					foreach($students as $student) {
						...
						foreach($lessons as $lesson) {
							if(in_array($lesson->ID, $student->completed_lessons)): $row .= __('Completed', 'perception');
							elseif(in_array($lesson->ID, $student->incomplete_lessons)): $row .= __('In progress', 'perception');
							else: $row .= __('Not started', 'perception'); 
							endif;
						   if($use_grading_system and !empty($student->relations[$lesson->ID]->grade)) {
						   		 $row .= " / ". sprintf(__('Grade: %s', 'perception'), $student->relations[$lesson->ID]->grade); 
						   }
							$row .=',';
						}		
						
						switch($student->perception_status):
							case 'pending': $row .= __('Pending', 'perception'); break;
							case 'enrolled': $row .= __('Enrolled', 'perception'); break;
							case 'rejected': $row .= __('Rejected', 'perception'); break;
							case 'completed': $row .= __('Completed', 'perception'); break;
							case 'frozen': $row .= __('Frozen', 'perception'); break;
						endswitch;
						
						if($use_grading_system) {
							$row .= ',' . (empty($student->grade) ? __('n/a', 'perception') : $student->grade);
						}
						$rows[] = $row;
					}
					...
					header('Content-Type: ' . perception_get_mime_type());
					header('Expires: ' . $now);
	   ...
	   wp_enqueue_style('thickbox.css', '/'.WPINC.'/js/thickbox/thickbox.css', null, '1.0');
		if(@file_exists(get_stylesheet_directory().'/perception/manage-students.php')) require get_stylesheet_directory().'/perception/manage-students.php';
		else require(PERCEPTION_PATH."/views/manage-students.php");
	}
	...
	static function lesson_status($student_id, $lesson_id, $status) {
      ...
      $student_id = intval($student_id);
		...
		$multiuser_access = 'all';
		$multiuser_access = PSPerceptionLMSMultiUser :: check_access('students_access');
		if( (!current_user_can('perception_manage') or $multiuser_access == 'view') 
				and $user_ID != $student_id) {
			wp_die(__("You cannot change someone else's status", 'perception'));
		}
			
		// if status == -1 we have to remove the existing record if any
		if($status == -1) {
			 $wpdb -> query($wpdb->prepare("DELETE FROM ".PERCEPTION_STUDENT_LESSONS." 
	  ...
	  if($status == 1) {
			...
			PSPerceptionLMSLessonModel :: complete($lesson_id, $student_id);
	  }		
		
		// set as 'in progress'
		if($status == 0 ) {
			 // record exists?
			 $exists = $wpdb->get_var($wpdb->prepare("SELECT id FROM ".PERCEPTION_STUDENT_LESSONS."
			 	WHERE student_id = %d AND lesson_id = %d", $student_id, $lesson_id));
	...
	static function cleanup($course_id, $student_id) {
      ...
      $course_id = intval($course_id);
		...
		$wpdb->query( $wpdb->prepare("DELETE FROM ".PERCEPTION_COURSES." 
		 	WHERE course_id = %d AND user_id=%d", $course_id, $student_id) );
		 	
		 $wpdb->query( $wpdb->prepare("DELETE FROM ".PERCEPTION_STUDENT_LESSONS." WHERE student_id= %d AND lesson_id IN (SELECT ID FROM {$wpdb->posts} tP JOIN {$wpdb->postmeta} tM ON tM.post_id = tP.ID AND tM.meta_key = 'perception_course' AND tM.meta_value = %d WHERE post_type = 'perception_lesson');", $student_id, $course_id) );					
		 
		// delete solutions
		$wpdb->query($wpdb->prepare("DELETE FROM ".PERCEPTION_STUDENT_HOMEWORKS." WHERE student_id=%d
			AND homework_id IN (SELECT id FROM ".PERCEPTION_HOMEWORKS." WHERE course_id=%d)", $student_id, $course_id)); 
			
		// delete homework notes
		$wpdb->query($wpdb->prepare("DELETE FROM ".PERCEPTION_HOMEWORK_NOTES." WHERE student_id=%d ", $student_id));	
		
		// cleanup module relations?
		if(get_option('perception_use_modules')) {
		    $wpdb->query( $wpdb->prepare("DELETE FROM ".PERCEPTION_STUDENT_MODULES." WHERE student_id= %d AND module_id IN 
		       (SELECT ID FROM {$wpdb->posts} tP JOIN {$wpdb->postmeta} tM ON tM.post_id = tP.ID AND tM.meta_key = 'perception_course' 
		       AND tM.meta_value = %d WHERE post_type = 'perception_module');", $student_id, $course_id) );					
		}
		
		do_action('perception_cleaned_student', $course_id, $student_id);
								 
		// cleanup exams data? 
		$use_exams = get_option('perception_use_exams');
		if($use_exams and get_option('perception_cleanup_exams') == 'yes') {
			// select all exam IDs of related lessons
			$_lesson = new PSPerceptionLMSLessonModel();
			$lessons = $_lesson->select($course_id);
			$exam_ids = array(0);
			foreach($lessons as $lesson) {
				$exam_id = get_post_meta($lesson->ID, 'perception_required_exam', true);
	...
	static function change_status($student_id, $course_id, $status) {
		...
		$current_status = $wpdb->get_var($wpdb->prepare("SELECT status FROM " . PERCEPTION_COURSES." 
			WHERE user_id=%d AND course_id=%d", $student_id, $course_id));
		if($current_status == $status) return false;	
		
		$wpdb->query($wpdb->prepare("UPDATE ".PERCEPTION_COURSES." SET
		...
		$course = get_post($course_id);
		 ...
		 if($status == 'enrolled') {
		 	do_action('perception_enrollment_approved',$student_id, $course_id);
		 	do_action('perception_admin_enrolled_course',$student_id, $course_id, true);
		 	$history_msg = sprintf(__('Enrollment in %s has been approved.','perception'), stripslashes($course->post_title));
		 }
		 else {
		 	do_action('perception_enrollment_rejected', $student_id, $course_id);
		 	$history_msg = sprintf(__('Enrollment in %s has been rejected.','perception'), stripslashes($course->post_title));
		 }	
